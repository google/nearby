...
void CloseFastPairController(FastPairController *pController) {
  NEARBY_LOGS(INFO) << "[[ Closing Fast Pair Service Adapter ]]";
  if (pController_ != nullptr) delete pController_;
  NEARBY_LOGS(INFO) << "[[ Successfully closed Fast Pair Service Adapter ]]";
}
...
void __stdcall StartScan(FastPairController *pController) {
  ...
  return static_cast<FastPairController *>(pController_)->StartScan();
}
