...
inline constexpr char kWifiAwareServiceIdMask = 0b00010000;
...
class WifiAwareConnectionInfo : public ConnectionInfo {
 public:
  ...
  friend bool operator==(const WifiAwareConnectionInfo& a,
                         const WifiAwareConnectionInfo& b) {
    return a.service_id_ == b.service_id_;
  }

  friend auto operator<=>(const WifiAwareConnectionInfo& a,
                          const WifiAwareConnectionInfo& b) {
    return a.service_id_ <=> b.service_id_;
  }
 ...
 private:
  ...
  const std::vector<uint8_t> actions_;
};
