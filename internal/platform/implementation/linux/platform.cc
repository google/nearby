...
#include "internal/platform/implementation/linux/ble.h"
...
ImplementationPlatform::CreateSingleThreadExecutor() {
  return absl::make_unique<linux::SingleThreadExecutor>();
}

std::unique_ptr<SubmittableExecutor>
ImplementationPlatform::CreateMultiThreadExecutor(int max_concurrency) {
  return absl::make_unique<linux::MultiThreadExecutor>(max_concurrency);
}
...
std::unique_ptr<WifiLanMedium> ImplementationPlatform::CreateWifiLanMedium() {
  return absl::make_unique<linux::WifiLanMedium>();
}

#ifndef NO_WEBRTC
std::unique_ptr<WebRtcMedium> ImplementationPlatform::CreateWebRtcMedium() {
  if (MediumEnvironment::Instance().GetEnvironmentConfig().webrtc_enabled) {
    return absl::make_unique<linux::WebRtcMedium>();
  } else {
    return nullptr;
  }
}
...
std::unique_ptr<Mutex> ImplementationPlatform::CreateMutex(Mutex::Mode mode) {
  if (mode == Mutex::Mode::kRecursive)
    return absl::make_unique<linux::RecursiveMutex>();
  else
    return absl::make_unique<linux::Mutex>(mode == Mutex::Mode::kRegular);
}
