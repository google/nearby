  ...
  network_utils::NetworkDiscoveredServiceCallback network_callback = {
      .network_service_discovered_cb =
          [this](const NsdServiceInfo& nsd_info) {
            service_callback_.service_discovered_cb(nsd_info);
          },
      .network_service_lost_cb =
          [this](const NsdServiceInfo& nsd_info) { service_callback_.service_lost_cb(nsd_info); }};
...
bool WifiAwareMedium::StartPublishing() {
  ...
  if (@available(iOS 26.0, *)) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      [awareSwiftWrapper getPairedDeviceCountWithCompletionHandler:^(NSInteger pairedDeviceCount) {
        GNCLoggerInfo(@"[NEARBY] Aware paired devices count: %@", @(pairedDeviceCount));
        if (pairedDeviceCount == 0) {
          dispatch_async(dispatch_get_main_queue(), ^{
            [awareSwiftWrapper showAwarePublishingViewAtTopWithCompletion:^{
              GNCLoggerInfo(
...
bool WifiAwareMedium::StartAdvertising(const NsdServiceInfo& nsd_service_info) { return false; }

bool WifiAwareMedium::StopAdvertising(const NsdServiceInfo& nsd_service_info) { return false; }
