...
- (void)setUp {
  [super setUp];
  _path =
      [NSTemporaryDirectory() stringByAppendingPathComponent:[[NSUUID UUID] UUIDString]].UTF8String;
  _preferencesManager = std::make_unique<nearby::apple::PreferencesManager>(_path);
}

- (void)tearDown {
  // Clear the user defaults for the test path.
  [[_preferencesManager->GetUserDefaults() persistentDomainForName:@(_path.c_str())]
      enumerateKeysAndObjectsUsingBlock:^(NSString* key, id obj, BOOL* stop) {
        [_preferencesManager->GetUserDefaults() removeObjectForKey:key];
      }];
...
- (void)testSaveAndGetBooleanArray {
  ...
  const bool kExpected[] = {true, false, true};
  XCTAssertEqual(_preferencesManager->GetBooleanArray(key, {}),
                 std::vector<bool>(std::begin(kExpected), std::end(kExpected)));
}
...
- (void)testGetBooleanArrayDefaultValue {
  ...
  XCTAssertEqual(
      _preferencesManager->GetBooleanArray(key, kDefaultValue),
