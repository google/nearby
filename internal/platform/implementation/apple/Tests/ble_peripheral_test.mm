...
// Fake GNCPeripheral for testing.
...
- (void)testIsNull {
  nearby::apple::BlePeripheral peripheral(nil);
  XCTAssertFalse(peripheral.GetPeripheral() != nil);
}
...
- (void)testGetPeripheral {
  ...
  FakePeripheral *fakePeripheral = [[FakePeripheral alloc] initWithIdentifier:uuid
                                                                         name:@"TestDevice"];
  nearby::apple::BlePeripheral peripheral(fakePeripheral);
  XCTAssertEqualObjects(peripheral.GetPeripheral(), fakePeripheral);
}
