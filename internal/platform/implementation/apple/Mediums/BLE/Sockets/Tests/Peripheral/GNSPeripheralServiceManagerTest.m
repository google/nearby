...
- (NSUInteger)maximumUpdateValueLength {
  return _maximumUpdateValueLength;
}
...
@interface GNSFakeCBATTRequest : NSObject
...
- (void)testSimpleGetters {
  ...
  GNSFakePeripheralManager *cbPeripheralManager =
      [[GNSFakePeripheralManager alloc] initWithDelegate:nil queue:nil options:nil];
...
- (void)testHandleConnectionConfirmPacket {
  ...
  [peripheralServiceManager handleConnectionConfirmPacket:packet context:nil];
  OCMVerifyAll(mockManager);
}
...
- (void)testHandleErrorPacket {
  ...
  [peripheralServiceManager handleErrorPacket:packet context:nil];
  OCMVerifyAll(mockManager);
}
