...
- (void)setUp {
  [super setUp];
  _mockSocket = OCMClassMock([GNSSocket class]);
...
- (void)testCreateConnectionWithServiceID {
  _connection = [GNCMBleConnection connectionWithSocket:_mockSocket
                                             serviceID:kServiceID
                                   expectedIntroPacket:NO
                                         callbackQueue:_callbackQueue];

  XCTAssertNotNil(_connection);
  OCMVerify([_mockSocket setDelegate:_connection]);
}
...
- (void)testSendData {
  ...
  XCTestExpectation *expectation = [self expectationWithDescription:@"Send completion"];

  OCMExpect([_mockSocket sendData:expectedPacket
                  progressHandler:[OCMArg any]
                       completion:[OCMArg any]])
      .andDo(^(NSInvocation *invocation) {
        __unsafe_unretained void (^completion)(NSError *error);
...
- (void)testReceiveData {
  ...
  id<GNSSocketDelegate> delegate = (id<GNSSocketDelegate>)_connection;
  [delegate socket:_mockSocket didReceiveData:packet];
...
- (void)testReceiveIntroPacketWithServiceID {
  ...
  id<GNSSocketDelegate> delegate = (id<GNSSocketDelegate>)_connection;
  [delegate socket:_mockSocket didReceiveData:introPacket];
...
- (void)testDisconnect {
  ...
  id<GNSSocketDelegate> delegate = (id<GNSSocketDelegate>)_connection;
  [delegate socket:_mockSocket didDisconnectWithError:nil];
