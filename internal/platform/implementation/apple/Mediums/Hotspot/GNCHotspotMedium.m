...
#import "internal/platform/implementation/apple/Mediums/Hotspot/GNCNWConnectionWrapper.h"
...
@implementation GNCHotspotMedium {
  ...
  id<GNCNWConnectionWrapping> (^_connectionWrapperFactory)(nw_connection_t);
}
...
- (instancetype)initWithQueue:(dispatch_queue_t)queue {
  ...
  if (self) {
    ...
    _connectionWrapperFactory = ^id<GNCNWConnectionWrapping>(nw_connection_t connection) {
      return [[GNCNWConnectionWrapper alloc] initWithNWConnection:connection];
    };
  }
  return self;
}
...
- (void)setConnectionWrapperFactory:(id<GNCNWConnectionWrapping> (^)(nw_connection_t))factory {
  _connectionWrapperFactory = factory;
}
...
- (nullable GNCHotspotSocket *)connectToEndpoint:(nw_endpoint_t)endpoint
  ...
  switch (blockResult) {
    ...
    case nw_connection_state_ready: {
      id<GNCNWConnectionWrapping> wrapper = _connectionWrapperFactory(connection);
      return [[GNCHotspotSocket alloc] initWithConnectionWrapper:wrapper];
    }
  }
}
