...
- (NSString *)getCurrentWifiSSID {
  ...
  // Wait for maximum 3 seconds for permission to be granted.
  for (int i = 0; i < 15; i++) {
    // Wait 0.2 seconds
    ...
    if (status == kCLAuthorizationStatusAuthorizedWhenInUse ||
      ...
      GNCLoggerInfo(@"✅ Location access granted");
      permissionGranted = YES;
      finished = YES;
      break;
    } else if (status == kCLAuthorizationStatusDenied ||
               status == kCLAuthorizationStatusRestricted) {
      GNCLoggerError(@"❌ Location access denied or restricted");
      finished = YES;
      break;
    }
  }
  ...
  if (!finished) {
    GNCLoggerError(@"❌ Location access permission check timeout");
  }
  if (!permissionGranted) return nil;

  GNCLoggerInfo(@"🔚 Location permission check complete. Continue program logic.");
