...
#import "internal/platform/implementation/apple/Mediums/Hotspot/Tests/GNCFakeNWConnectionWrapper.h"
...
@implementation GNCHotspotSocketTests {
  GNCFakeNWConnectionWrapper *_fakeConnectionWrapper;
  GNCHotspotSocket *_socket;
}

- (void)setUp {
  [super setUp];
  _fakeConnectionWrapper = [[GNCFakeNWConnectionWrapper alloc] init];
  _socket = [[GNCHotspotSocket alloc] initWithConnectionWrapper:_fakeConnectionWrapper];
}

- (void)tearDown {
  _socket = nil;
  _fakeConnectionWrapper = nil;
  [super tearDown];
}
...
- (void)testReadMaxLength_Success {
  ...
  _fakeConnectionWrapper.dataToReceive = (dispatch_data_t)testData;
...
- (void)testReadMaxLength_Error {
  NSError *error = nil;
  _fakeConnectionWrapper.simulateReceiveFailure = YES;
...
- (void)testWrite_Error {
  ...
  NSString *testString = @"testData";
  NSData *testData = [testString dataUsingEncoding:NSUTF8StringEncoding];
  _fakeConnectionWrapper.simulateSendFailure = YES;
...
- (void)testClose {
  XCTAssertFalse(_fakeConnectionWrapper.cancelCalled);
  [_socket close];
  XCTAssertTrue(_fakeConnectionWrapper.cancelCalled);
