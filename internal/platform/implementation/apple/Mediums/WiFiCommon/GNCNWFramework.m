...
// The default browse result wrapper.
static id<GNCNWBrowseResult> gBrowseResultWrapper;
static id<GNCNWBrowser> gNWBrowserWrapper;
static id<GNCNWConnection> gNWConnectionWrapper;
...
// Test-only function to inject a fake browse result wrapper.
void GNCSetBrowseResultWrapper(id<GNCNWBrowseResult> wrapper) { gBrowseResultWrapper = wrapper; }

// Test-only function to inject a fake NWBrowser wrapper.
void GNCSetNWBrowserWrapper(id<GNCNWBrowser> wrapper) { gNWBrowserWrapper = wrapper; }

// Test-only function to inject a fake NWConnection wrapper.
void GNCSetNWConnectionWrapper(id<GNCNWConnection> _Nullable wrapper) {
  gNWConnectionWrapper = wrapper;
}

BOOL GNCHasLoopbackInterface(nw_browse_result_t browseResult) {
  __block BOOL isLoopback = NO;
  id<GNCNWBrowseResult> wrapper = gBrowseResultWrapper ?: [[GNCNWBrowseResultImpl alloc] init];
...
- (BOOL)startDiscoveryForServiceType:(NSString *)serviceType
  ...
  id<GNCNWBrowser> browserWrapper = gNWBrowserWrapper ?: [[GNCNWBrowserImpl alloc] init];
      ...
      setBrowseResultsChangedHandler:browser
                             handler:^(nw_browse_result_t old_result, nw_browse_result_t new_result,
                               ...
                               id<GNCNWBrowseResult> browseResultWrapper =
                                   gBrowseResultWrapper ?: [[GNCNWBrowseResultImpl alloc] init];
...
- (nullable GNCNWFrameworkSocket *)internalConnectToEndpoint:(nw_endpoint_t)endpoint
  ...
  id<GNCNWConnection> connectionWrapper =
      gNWConnectionWrapper ?: [[GNCNWConnectionImpl alloc] init];
  nw_connection_t connection = [connectionWrapper createWithEndpoint:endpoint
                                                          parameters:parameters];
  [connectionWrapper setQueue:connection queue:queue ? queue : _dispatchQueue];
  [connectionWrapper
      setStateChangedHandler:connection
  ...
  if (cancelSource) {
    dispatch_source_set_event_handler(cancelSource, ^{
      ...
      [connectionWrapper cancel:connection];
      dispatch_source_cancel(cancelSource);
    });
  }
  [connectionWrapper start:connection];
  ...
  switch (blockResult) {
    ...
    case nw_connection_state_ready: {
      id<GNCNWConnection> connectionObj;
      if (gNWConnectionWrapper) {
        connectionObj = gNWConnectionWrapper;
      } else {
        connectionObj = [[GNCNWConnectionImpl alloc] initWithNWConnection:connection];
      }
      return [[GNCNWFrameworkSocket alloc] initWithConnection:connectionObj];
    }
  }
}
