...
- (void)discoverCharacteristicsWithUUIDs:(NSArray<CBUUID *> *)characteristicUUIDs
  ...
  dispatch_async(_queue, ^{
    ...
    if (dispatch_semaphore_wait(_semaphore, timeout) != 0) {
      ...
      [_dataLock lock];
      _currentDiscoverCharacteristicsRequest.completionHandler(GATTTimeoutError());
      _currentDiscoverCharacteristicsRequest = nil;
      [_dataLock unlock];
    }
  });
...
- (void)readValueForCharacteristic:(GNCBLEGATTCharacteristic *)characteristic
  ...
  dispatch_async(_queue, ^{
    ...
    if (dispatch_semaphore_wait(_semaphore, timeout) != 0) {
      ...
      [_dataLock lock];
      _currentReadCharacteristicValueRequest.completionHandler(nil, GATTTimeoutError());
      _currentReadCharacteristicValueRequest = nil;
      [_dataLock unlock];
    }
  });
...
- (void)gnc_peripheral:(id<GNCPeripheral>)peripheral didDiscoverServices:(nullable NSError *)error {
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
    ...
    if (_discoveredServicesCount == 0) {
      _currentDiscoverCharacteristicsRequest.completionHandler(InvalidServiceDataError());
      _currentDiscoverCharacteristicsRequest = nil;
      [_dataLock unlock];
      dispatch_semaphore_signal(_semaphore);
      return;
...
- (void)gnc_peripheral:(id<GNCPeripheral>)peripheral
  ...
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
    ...
    if ([[NSSet setWithArray:_currentDiscoverCharacteristicsRequest.characteristicUUIDs]
      ...
      _currentDiscoverCharacteristicsRequest.completionHandler(nil);
      _currentDiscoverCharacteristicsRequest = nil;
      [_dataLock unlock];
      dispatch_semaphore_signal(_semaphore);
      return;
    ...
    if (_didDiscoverCharacteristicsCount == _discoveredServicesCount) {
      _currentDiscoverCharacteristicsRequest.completionHandler(InvalidCharacteristicError());
      _currentDiscoverCharacteristicsRequest = nil;
      [_dataLock unlock];
      dispatch_semaphore_signal(_semaphore);
      return;
...
- (void)gnc_peripheral:(id<GNCPeripheral>)peripheral
  ...
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
    ...
    _currentReadCharacteristicValueRequest.completionHandler(characteristic.value, error);
    _currentReadCharacteristicValueRequest = nil;
    [_dataLock unlock];
    dispatch_semaphore_signal(_semaphore);
  });
