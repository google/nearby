...
- (NSUInteger)processReceivedPacket {
  ...
  if (GNCFeatureFlags.refactorBleL2capEnabled) {
    if (_connectionHandlers.payloadHandler) {
      dispatch_async(_callbackQueue, ^{
        // TODO: edwinwu - Filter out the kL2CAPPacketLength in the packet is the logic in
        // connection layer . Move it to upper layer.
        _connectionHandlers.payloadHandler(
            [NSData dataWithData:[realData subdataWithRange:NSMakeRange(0, realData.length)]]);
      });
    }
  } else {
