...
- (void)testGNSErrorWithCode {
  ...
  NSError *error = GNSErrorWithCode(GNSErrorNoConnection);
  ...
  // Test another known error code
  error = GNSErrorWithCode(GNSErrorConnectionTimedOut);
  XCTAssertEqualObjects(error.domain, kGNSSocketsErrorDomain);
  XCTAssertEqual(error.code, GNSErrorConnectionTimedOut);
  XCTAssertNotNil(error.localizedDescription);
