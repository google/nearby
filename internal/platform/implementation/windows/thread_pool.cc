...
VOID CALLBACK TimerCallback(PTP_CALLBACK_INSTANCE instance, PVOID context,
                            PTP_TIMER timer) {
  // Instance is not used in tiemr callbacks.
  UNREFERENCED_PARAMETER(instance);

  VLOG(1) << __func__ << ": Start to run timer(" << timer << ") callback.";
  ThreadPool* thread_pool = reinterpret_cast<ThreadPool*>(context);
...
std::optional<uint64_t> ThreadPool::Run(Runnable task, absl::Duration delay,
  ...
  delayed_task_map_.put(timer, std::make_unique<DelayedTaskInfo>(
                                   DelayedTaskInfo{.task = std::move(task),
                                                   .delay = delay,
                                                   .period = period,
                                                   .is_done = false}));
...
void ThreadPool::TaskQueue::empty() {
  absl::MutexLock lock(&mutex_);
  queue_.empty();
}
...
void ThreadPool::DelayedTaskMap::empty() {
  absl::MutexLock lock(&mutex_);
  task_map_.clear();
}
