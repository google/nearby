...
#include "internal/platform/implementation/windows/bluetooth_gatt_client.h"
...
BluetoothGattClient::BluetoothGattClient(BluetoothLEDevice& ble_device)
    : ble_device_(ble_device) {}

void BluetoothGattClient::StartGattDiscovery() {
...
void BluetoothGattClient::OnGetGattServices(
...
void BluetoothGattClient::OnServiceOpen(GattDeviceService& gatt_service,
...
void BluetoothGattClient::OnGetCharacteristics(
    uint16_t service_attribute_handle,
    GattCharacteristicsResult& characteristics_result) {
...
BluetoothGattClient::GattCharacteristicList&
BluetoothGattClient::GetCharacteristics(std::string_view service_uuid) {
...
void BluetoothGattClient::WriteCharacteristic(
    std::string_view service_uuid,
    std::string_view characteristic_uuid, const ByteArray& value) {
...
void BluetoothGattClient::WriteCharacteristic(
    GattCharacteristic& characteristics, const ByteArray& value) {
  ...
  OnWriteValueWithResultAndOption(result);
}

void BluetoothGattClient::OnWriteValueWithResultAndOption(
...
void BluetoothGattClient::SubscribeToNotifications(
...
void BluetoothGattClient::UnsubscribeFromNotifications(
    GattCharacteristic& characteristics) {
  if (WriteCccDescriptor(
          characteristics,
...
bool BluetoothGattClient::WriteCccDescriptor(
    GattCharacteristic& characteristics,
...
GattClientCharacteristicConfigurationDescriptorValue
BluetoothGattClient::GetCccdValue(GattCharacteristicProperties properties) {
...
void BluetoothGattClient::AddValueChangedHandler(
    GattCharacteristic& characteristics) {
  if (!notifications_token_) {
    NEARBY_LOGS(VERBOSE)
        << " notifications_token_ = characteristics.ValueChanged";
    notifications_token_ = characteristics.ValueChanged(
        {this, &BluetoothGattClient::OnValueChanged});
  }
}
void BluetoothGattClient::RemoveValueChangedHandler(
    GattCharacteristic& characteristics) {
  if (notifications_token_) {
    characteristics.ValueChanged(std::exchange(notifications_token_, {}));
  }
}
void BluetoothGattClient::OnValueChanged(
