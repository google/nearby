...
BleV2Medium::AdvertisementWatcher::~AdvertisementWatcher() {
  Stop();
}
...
void BleV2Medium::AdvertisementWatcher::Initialize(
    ...
    const winrt::Windows::Foundation::TypedEventHandler<
        BluetoothLEAdvertisementWatcher,
        BluetoothLEAdvertisementWatcherStoppedEventArgs>& stopped_handler) {
...
bool BleV2Medium::AdvertisementWatcher::Start() {
  ...
  try {
    ...
    LOG(ERROR) << __func__
               << ": Exception on xBLE scan start: " << exception.what();
  } catch (const winrt::hresult_error& ex) {
    LOG(ERROR) << __func__
               << ": Exception on  BLE scan start: " << ex.code() << ": "
               << winrt::to_string(ex.message());
  } catch (...) {
...
void BleV2Medium::AdvertisementWatcher::Stop() {
  ...
  try {
    ...
    LOG(ERROR) << __func__
               << ": Exception on BLE scan stop: " << ex.code() << ": "
               << winrt::to_string(ex.message());
  } catch (...) {
...
BleV2Medium::CreateBleWatcher(uint16_t service_uuid) {
  ...
  watcher->Initialize(service_uuid,
                      {this, &BleV2Medium::AdvertisementReceivedHandler},
  ...
  LOG(INFO) << __func__
            << ": BLE scanning started for service uuid " << std::hex
            << service_uuid;
  return watcher;
}
...
std::unique_ptr<BleV2Medium::ScanningSession> BleV2Medium::StartScanning(
  ...
  if (watchers_.empty()) {
    ...
    auto watcher = std::make_unique<AdvertisementWatcher>();
    watcher->Initialize(
        {this, &BleV2Medium::AdvertisementFoundHandler},
        {this, &BleV2Medium::WatcherHandler});
    if (!watcher->Start()) {
      return nullptr;
    }
