...
void CredentialStorageImpl::SaveCredentials(
  ...
  // Define save_public_credentials_callback function.
  ...
  // Define save_private_credentials_callback function.
...
void CredentialStorageImpl::SavePrivateCredentials(
  ...
  NEARBY_LOGS(INFO) << "G3 Save Private Credentials for account: "
                    << account_name;
  auto key_value = std::make_pair(std::make_tuple(manager_app_id, account_name),
                                  private_credentials);
  auto res = private_credentials_map_.insert(key_value);
  if (!res.second) {
    NEARBY_LOGS(WARNING)
        << "Credentials already saved in map. Overriding previous creds!";
    private_credentials_map_[std::make_tuple(manager_app_id, account_name)] =
        private_credentials;
  }
...
void CredentialStorageImpl::GetPrivateCredentials(
  ...
  auto key = std::make_tuple(credential_selector.manager_app_id,
                             credential_selector.account_name);
...
CredentialStorageImpl::GetPrivateCredentialsForTesting(
    const std::tuple<absl::string_view, absl::string_view>& key) {
  return private_credentials_map_[key];
}

std::vector<PublicCredential>
CredentialStorageImpl::GetPublicCredentialsForTesting(
    const std::tuple<absl::string_view, absl::string_view,
                     api::PublicCredentialType>& key) {
  return public_credentials_map_[key];
}
