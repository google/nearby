...
bool WifiDirectMedium::ConnectWifiDirect(
  ...
  if (!remote_medium) {
     env.UpdateWifiDirectMediumForStartOrConnect(*this, wifi_direct_credentials,
                                        /*is_go=*/false, /*enabled=*/false);
    return false;
  }

  env.UpdateWifiDirectMediumForStartOrConnect(*this, wifi_direct_credentials,
                                        /*is_go=*/false, /*enabled=*/true);
  return true;
}
...
std::unique_ptr<api::WifiHotspotSocket> WifiDirectMedium::ConnectToService(
  ...
  auto& env = MediumEnvironment::Instance();
  auto* remote_medium = static_cast<WifiDirectMedium*>(
      env.GetWifiDirectMedium({}, ip_address));
  ...
  CancellationFlagListener listener(cancellation_flag, [&server_socket]() {
    NEARBY_LOGS(INFO) << "G3 WifiDirect Cancel Connect.";
    if (server_socket != nullptr) {
      server_socket->Close();
    }
  });
  ...
  if (!server_socket->Connect(*socket)) {
    NEARBY_LOGS(ERROR)
        << "G3 WifiDirect GC Failed to connect to existing WifiDirect GO"
           "Server socket: name="
        << socket_name;
    return {};
  }
