...
// A unique identifier assigned to each discovered BLE peripheral.
// Any metadata associated with the peripheral, e.g. MAC address, are maintained
// in the Medium privately.
// Some medium may choose to store a packed MAC address in the PeripheralId e.g.
// see `MacAddress::ToString()`.  This is platform dependent and the framework
// assigns no meaning to the `PeripheralId`.
using PeripheralId = uint64_t;
...
class BlePeripheral {
 public:
  // Legacy alias for `PeripheralId`.
  using UniqueId = PeripheralId;

  BlePeripheral(PeripheralId unique_id, MacAddress address)
      : unique_id_(unique_id), address_(std::move(address)) {}
  explicit BlePeripheral(PeripheralId unique_id = 0)
  ...
  virtual PeripheralId GetUniqueId() const { return unique_id_; }
...
struct ServerGattConnectionCallback {
  ...
  absl::AnyInvocable<void(const PeripheralId remote_device_id,
...
class BleSocket {
 public:
  ...
  // Returns the PeripheralId that is connected to this socket.
  virtual PeripheralId GetRemotePeripheralId() = 0;
};
...
class BleL2capSocket {
 public:
  ...
  // Returns valid PeripheralId if there is a connection.
  virtual PeripheralId GetRemotePeripheralId() = 0;
};
...
class BleMedium {
 public:
  ...
  struct ScanCallback {
    absl::AnyInvocable<void(PeripheralId peripheral_id,
                            BleAdvertisementData advertisement_data)>
        advertisement_found_cb =
            [](PeripheralId, BleAdvertisementData) {};
  };
  ...
  struct ScanningCallback {
    ...
    absl::AnyInvocable<void(PeripheralId peripheral_id,
        ...
        advertisement_found_cb =
            [](PeripheralId, BleAdvertisementData) {};
    absl::AnyInvocable<void(PeripheralId peripheral_id)>
        advertisement_lost_cb = [](PeripheralId) {};
  };
  ...
  virtual std::unique_ptr<GattClient> ConnectToGattServer(
      PeripheralId peripheral_id, TxPowerLevel tx_power_level,
