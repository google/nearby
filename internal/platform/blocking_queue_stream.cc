...
BlockingQueueStream::BlockingQueueStream() {
  NEARBY_LOGS(INFO) << __func__ << "Create a BlockingQueueStream with size"
...
ExceptionOr<ByteArray> BlockingQueueStream::Read(std::int64_t size) {
  NEARBY_LOGS(INFO) << __func__ << "Read " << size << " bytes";
  return ExceptionOr<ByteArray>();
}

ExceptionOr<ByteArray> BlockingQueueStream::Read() {
...
void BlockingQueueStream::Write(const ByteArray& bytes) {
  ...
  is_writing_ = false;
  NEARBY_LOGS(INFO) << "BlockingQueueStream wrote " << bytes.size() << " bytes";
}
...
Exception BlockingQueueStream::Close() {
  ...
  is_closed_ = true;
  NEARBY_LOGS(INFO) << __func__ << "Close BlockingQueueStream.";
  if (is_writing_) {
    NEARBY_LOGS(INFO)
