...
TEST_P(ScheduledExecutorTest, ConsructorDestructorWorks) {
  ScheduledExecutor executor;
}

TEST_P(ScheduledExecutorTest, CanExecute) {
  absl::Mutex mutex;
  absl::CondVar cond;
...
TEST_P(ScheduledExecutorTest, CanSchedule) {
  ScheduledExecutor executor;
...
TEST_P(ScheduledExecutorTest, CanCancel) {
  ScheduledExecutor executor;
  std::atomic_int value = 0;
...
TEST_P(ScheduledExecutorTest, CanCancelTwice) {
  ScheduledExecutor executor;
  std::atomic_int value = 0;
  Cancelable cancelable =
      executor.Schedule([&value]() { value += 1; }, kShortDelay);
...
TEST_P(ScheduledExecutorTest, FailToCancel) {
  absl::Mutex mutex;
  absl::CondVar cond;
  ScheduledExecutor executor;
  std::atomic_int value = 0;
...
TEST_P(ScheduledExecutorTest, ShutdownWaitsForRunningTasks) {
  ScheduledExecutor executor;
  std::atomic_int value = 0;
  executor.Execute([&]() {
    absl::SleepFor(kLongDelay);
...
TEST_P(ScheduledExecutorTest, ExecuteAfterShutdownFails) {
  ScheduledExecutor executor;
...
TEST_P(ScheduledExecutorTest, ExecuteDuringShutdownFails) {
...
TEST_P(ScheduledExecutorTest, SimulatedClockCanSchedule) {
...
TEST_P(ScheduledExecutorTest, ThreadCheck_Execute) {
...
TEST_P(ScheduledExecutorTest, ThreadCheck_Schedule) {
  ThreadCheckTestClass test_class;
  absl::Notification notification;
