...
class FakeWifiAwareMedium : public api::WifiAwareMedium {
 public:
  ...
  void TriggerDiscovered(const std::string& service_type,
                         const WifiAwareServiceInfo& info) {
...
TEST(WifiAwareMediumTest, DiscoveryFlow) {
  ...
  WifiAwareMedium::DiscoveredServiceCallback callback = {
      .service_discovered_cb =
          [&](WifiAwareServiceInfo info, absl::string_view type) {
  ...
  // Trigger discovery for unknown service type in WifiAwareServiceInfo (should
  // be safely ignored).
  WifiAwareServiceInfo unknown_type_info;
  ...
  // Trigger service lost for unknown service type in WifiAwareServiceInfo
  ...
  WifiAwareServiceInfo info_unknown;
...
TEST(WifiAwareMediumTest, StartDiscoveryFails) {
  auto fake_impl = std::make_unique<FakeWifiAwareMedium>();
  FakeWifiAwareMedium* fake_ptr = fake_impl.get();
  fake_ptr->SetDiscoveryFail(true);
  WifiAwareMedium medium(std::move(fake_impl));

  WifiAwareMedium::DiscoveredServiceCallback callback;
  EXPECT_FALSE(medium.StartDiscovery("_fail._tcp", std::move(callback)));

  // Verify that insertion was properly reverted and subsequent start succeeds.
  fake_ptr->SetDiscoveryFail(false);
  WifiAwareMedium::DiscoveredServiceCallback callback2;
  EXPECT_TRUE(medium.StartDiscovery("_fail._tcp", std::move(callback2)));
}
...
TEST(WifiAwareServiceInfoTest, BasicPropertiesAndConversion) {
  WifiAwareServiceInfo default_info;
  EXPECT_FALSE(default_info.IsValid());
  EXPECT_EQ(default_info.GetPort(), 0);
  EXPECT_TRUE(default_info.GetServiceName().empty());
  EXPECT_TRUE(default_info.GetServiceType().empty());
  EXPECT_TRUE(default_info.GetTxtRecords().empty());

  WifiAwareServiceInfo info;
  info.SetServiceName("service_name_123");
  info.SetServiceType("_nearby_aware._tcp");
  info.SetPort(8080);
  info.SetIPAddress("192.168.1.1");
  info.SetIPv6Address("fe80::1");
  info.SetTxtRecord("key1", "val1");
  info.SetTxtRecord("key2", "val2");

  EXPECT_TRUE(info.IsValid());
  EXPECT_EQ(info.GetServiceName(), "service_name_123");
  EXPECT_EQ(info.GetServiceType(), "_nearby_aware._tcp");
  EXPECT_EQ(info.GetPort(), 8080);
  EXPECT_EQ(info.GetIPAddress(), "192.168.1.1");
  EXPECT_EQ(info.GetIPv6Address(), "fe80::1");
  EXPECT_EQ(info.GetTxtRecord("key1"), "val1");
  EXPECT_EQ(info.GetTxtRecord("key2"), "val2");
  EXPECT_TRUE(info.GetTxtRecord("nonexistent").empty());
  EXPECT_EQ(info.GetTxtRecords().size(), 2);

  // Test conversion to NsdServiceInfo
  auto nsd_info = static_cast<NsdServiceInfo>(info);
  EXPECT_EQ(nsd_info.GetServiceName(), "service_name_123");
  EXPECT_EQ(nsd_info.GetServiceType(), "_nearby_aware._tcp");
  EXPECT_EQ(nsd_info.GetPort(), 8080);
  EXPECT_EQ(nsd_info.GetIPAddress(), "192.168.1.1");
  EXPECT_EQ(nsd_info.GetIPv6Address(), "fe80::1");
  EXPECT_EQ(nsd_info.GetTxtRecord("key1"), "val1");
  EXPECT_EQ(nsd_info.GetTxtRecord("key2"), "val2");

  // Test construction from NsdServiceInfo
  WifiAwareServiceInfo from_nsd(nsd_info);
  EXPECT_TRUE(from_nsd.IsValid());
  EXPECT_EQ(from_nsd.GetServiceName(), "service_name_123");
  EXPECT_EQ(from_nsd.GetServiceType(), "_nearby_aware._tcp");
  EXPECT_EQ(from_nsd.GetPort(), 8080);
  EXPECT_EQ(from_nsd.GetIPAddress(), "192.168.1.1");
  EXPECT_EQ(from_nsd.GetIPv6Address(), "fe80::1");
  EXPECT_EQ(from_nsd.GetTxtRecord("key1"), "val1");
  EXPECT_EQ(from_nsd.GetTxtRecord("key2"), "val2");
}
