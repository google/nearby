...
BwuManager::ProcessBwuPathAvailableEventInternal(
  ...
  bool disble_ble_scanning = false;
  {
  ...
  if (old_medium == Medium::BLE && medium == Medium::WIFI_HOTSPOT) {
    NEARBY_LOGS(INFO) << "If upgrade from BLE to WIFI_HOTSPOT, we need to stop "
                         "BLE scanning because it can interfere with WIFI "
                         "Hotspot scanning and connection.";
    disble_ble_scanning = true;
