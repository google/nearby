...
TEST(BleL2capPacketTest, ByteArrayForRequestAdvertisementWithEmptyServiceID) {
  std::optional<ByteArray> byte_array =
      BleL2capPacket::ByteArrayForRequestAdvertisement("");

  ASSERT_FALSE(byte_array.has_value());
}
...
TEST(BleL2capPacketTest,
  ...
  ASSERT_EQ(result.value().GetAdvertisement().value(), advertisement);
}
