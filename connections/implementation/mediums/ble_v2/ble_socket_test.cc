...
TEST_P(BleSocketTest, ProcessIncomingL2capPacketValidationSucceeds) {
  BleSocketTestParams params = GetParam();
  if (!params.is_l2cap_socket) {
    return;
  }

  // Prepare the input stream to contain a data connection request.
  ByteArray request_data_connection =
      BleL2capPacket::ByteArrayForRequestDataConnection();
  auto socket =
      CreateTestSocket(params.is_l2cap_socket, request_data_connection);
  ASSERT_NE(socket, nullptr);

  Exception result = socket->ProcessIncomingL2capPacketValidation();

  EXPECT_TRUE(result.Ok());
}

TEST_P(BleSocketTest, ProcessOutgoingL2capPacketValidationSucceeds) {
  BleSocketTestParams params = GetParam();
  if (!params.is_l2cap_socket) {
    return;
  }

  // Prepare the input stream with the expected data connection ready response.
  ByteArray data_connection_ready =
      BleL2capPacket::ByteArrayForDataConnectionReady();
  auto socket = CreateTestSocket(params.is_l2cap_socket, data_connection_ready);
  ASSERT_NE(socket, nullptr);

  Exception result = socket->ProcessOutgoingL2capPacketValidation();
