...
class BleInputStream : public InputStream {
 public:
  BleInputStream(InputStream& source, const ByteArray& service_id_hash)
...
class BleSocket final {
 public:
  ...
  static std::unique_ptr<BleSocket> CreateWithBleSocket(
    ...
    return std::unique_ptr<BleSocket>(
        new BleSocket(service_id_hash,
                      std::make_unique<BleInputStream>(
                          ble_socket.GetInputStream(), service_id_hash),
                      std::make_unique<BleOutputStream>(
                          ble_socket.GetOutputStream(), service_id_hash),
                      std::move(ble_socket)));
  }
  ...
  static std::unique_ptr<BleSocket> CreateWithL2capSocket(
    ...
    return std::unique_ptr<BleSocket>(
        new BleSocket(service_id_hash,
                      std::make_unique<BleInputStream>(
                          l2cap_socket.GetInputStream(), service_id_hash),
                      std::make_unique<BleOutputStream>(
                          l2cap_socket.GetOutputStream(), service_id_hash),
