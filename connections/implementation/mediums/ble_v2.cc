...
ErrorOr<bool> BleV2::StartAcceptingL2capConnections(
  ...
  MutexLock lock(&mutex_);
  if (service_id.empty()) {
    LOG(INFO) << "Refusing to start accepting BLE L2CAP connections with empty "
                 "service id.";
    return {Error(OperationResultCode::CLIENT_BLE_DUPLICATE_DISCOVERING)};
  }
...
ErrorOr<BleL2capSocket> BleV2::ConnectOverL2cap(
  ...
  if (service_id.empty()) {
    LOG(INFO) << "Refusing to create client Ble L2CAP socket because "
                 "service_id is empty.";
    return {Error(OperationResultCode::NEARBY_LOCAL_CLIENT_STATE_WRONG)};
  }
  ...
  if (cancellation_flag->Cancelled()) {
    LOG(INFO) << "Can't create client Ble L2CAP socket due to cancel.";
    return {Error(OperationResultCode::
                      CLIENT_CANCELLATION_CANCEL_BLE_OUTGOING_CONNECTION)};
  }
  ...
  if (!socket.IsValid()) {
    LOG(INFO) << "Failed to Connect via BleL2cap [service_id=" << service_id
              << "]";
    return {Error(OperationResultCode::
