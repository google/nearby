...
bool Awdl::StopAdvertising(const std::string& service_id) {
  ...
  if (!IsAdvertisingLocked(service_id)) {
    NEARBY_LOGS(INFO) << "Can't turn off Awdl advertising; it is already off";
    return false;
  }
...
ErrorOr<bool> Awdl::StartAcceptingConnections(
  ...
  if (!IsAvailableLocked()) {
    NEARBY_LOGS(INFO) << "Can't start accepting Awdl connections [service_id="
...
ErrorOr<AwdlSocket> Awdl::Connect(const std::string& service_id,
  ...
  if (!socket.IsValid()) {
    NEARBY_LOGS(INFO) << "Failed to Connect via Awdl [service_id=" << service_id
                      << "]";
    return {Error(
