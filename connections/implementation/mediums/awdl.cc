...
ErrorOr<bool> Awdl::StartAdvertising(const std::string& service_id,
  ...
  if (!IsAvailableLocked()) {
    NEARBY_LOGS(INFO)
        << "Can't turn on Awdl advertising. Awdl is not available.";
  ...
  if (IsAdvertisingLocked(service_id)) {
    NEARBY_LOGS(INFO)
        << "Failed to Awdl advertise because we're already advertising.";
    return {Error(OperationResultCode::CLIENT_WIFI_LAN_DUPLICATE_ADVERTISING)};
  }

  if (!IsAcceptingConnectionsLocked(service_id)) {
    NEARBY_LOGS(INFO)
        << "Failed to turn on Awdl advertising with nsd_service_info="
        << &nsd_service_info
  ...
  NEARBY_LOGS(INFO) << "Turned on Awdl advertising with nsd_service_info="
                    << &nsd_service_info
                    << ", service_name=" << nsd_service_info.GetServiceName()
                    << ", service_id=" << service_id;
...
bool Awdl::StopAdvertising(const std::string& service_id) {
  ...
  if (!IsAdvertisingLocked(service_id)) {
    NEARBY_LOGS(INFO)
        << "Can't turn off Awdl advertising; it is already off";
    return false;
  }

  NEARBY_LOGS(INFO) << "Turned off Awdl advertising with service_id="
                    << service_id;
  bool ret =
...
ErrorOr<bool> Awdl::StartDiscovery(const std::string& service_id,
  ...
  if (service_id.empty()) {
    NEARBY_LOGS(INFO)
        << "Refusing to start Awdl discovering with empty service_id.";
    return {Error(OperationResultCode::NEARBY_LOCAL_CLIENT_STATE_WRONG)};
  }

  if (!IsAvailableLocked()) {
    NEARBY_LOGS(INFO)
        << "Can't discover Awdl services because Awdl isn't available.";
    return {Error(
  ...
  if (IsDiscoveringLocked(service_id)) {
    NEARBY_LOGS(INFO)
        << "Refusing to start discovery of Awdl services because another "
  ...
  if (!ret) {
    NEARBY_LOGS(INFO) << "Failed to start discovery of Awdl services.";
    return {Error(
        OperationResultCode::CONNECTIVITY_WIFI_LAN_START_DISCOVERY_FAILURE)};
  }

  NEARBY_LOGS(INFO) << "Turned on Awdl discovering with service_id="
                    << service_id;
...
bool Awdl::StopDiscovery(const std::string& service_id) {
  ...
  if (!IsDiscoveringLocked(service_id)) {
    NEARBY_LOGS(INFO)
        << "Can't turn off Awdl discovering because we never started "
           "discovering.";
    return false;
  }

  std::string service_type = GenerateServiceType(service_id);
  NEARBY_LOGS(INFO) << "Turned off Awdl discovering with service_id="
...
ErrorOr<bool> Awdl::StartAcceptingConnections(
  ...
  if (service_id.empty()) {
    NEARBY_LOGS(INFO) << "Refusing to start accepting Awdl connections; "
                         "service_id is empty.";
    return {Error(OperationResultCode::NEARBY_LOCAL_CLIENT_STATE_WRONG)};
  }
  ...
  if (!IsAvailableLocked()) {
    NEARBY_LOGS(INFO)
        << "Can't start accepting Awdl connections [service_id="
  ...
  if (IsAcceptingConnectionsLocked(service_id)) {
    NEARBY_LOGS(INFO)
        << "Refusing to start accepting Awdl connections [service="
        << service_id
  ...
  if (!server_socket.IsValid()) {
    NEARBY_LOGS(INFO)
        << "Failed to start accepting Awdl connections for service_id="
        << service_id;
    return {Error(OperationResultCode::
...
bool Awdl::StopAcceptingConnections(const std::string& service_id) {
  ...
  if (service_id.empty()) {
    NEARBY_LOGS(INFO) << "Unable to stop accepting Awdl connections because "
  ...
  const auto& it = server_sockets_.find(service_id);
  if (it == server_sockets_.end()) {
    NEARBY_LOGS(INFO) << "Can't stop accepting Awdl connections for "
  ...
  if (!listening_socket.Close().Ok()) {
    NEARBY_LOGS(INFO) << "Failed to close Awdl server socket for service_id="
                      << service_id;
    return false;
  }
...
ErrorOr<AwdlSocket> Awdl::Connect(const std::string& service_id,
  ...
  if (service_id.empty()) {
    NEARBY_LOGS(INFO) << "Refusing to create client Awdl socket because "
                         "service_id is empty.";
    return {Error(OperationResultCode::NEARBY_LOCAL_CLIENT_STATE_WRONG)};
  }

  if (!IsAvailableLocked()) {
    NEARBY_LOGS(INFO) << "Can't create client Awdl socket [service_id="
                      << service_id << "]; Awdl isn't available.";
    return {Error(OperationResultCode::MEDIUM_UNAVAILABLE_LAN_NOT_AVAILABLE)};
  }

  if (cancellation_flag->Cancelled()) {
    NEARBY_LOGS(INFO) << "Can't create client Awdl socket due to cancel.";
    return {Error(OperationResultCode::
  ...
  if (!socket.IsValid()) {
    NEARBY_LOGS(INFO) << "Failed to Connect via Awdl [service_id="
                      << service_id << "]";
    return {Error(
    ...
    if (virtual_socket.ok()) {
      NEARBY_LOGS(INFO)
          << "Successfully connected via Multiplex Awdl [service_id="
          << service_id << "]";
      return virtual_socket.result();
    }
  }

  NEARBY_LOGS(INFO) << "Successfully connected via Awdl [service_id="
                    << service_id << "]";
  return socket;
}
    ...
    if (it != multiplex_sockets_.end()) {
      ...
      if (multiplex_socket->IsEnabled()) {
        ...
        if (wlan_socket == nullptr) {
          NEARBY_LOGS(INFO) << "Failed to cast to AwdlSocket for "
...
ExceptionOr<AwdlSocket> Awdl::CreateOutgoingMultiplexSocketLocked(
  ...
  if (is_multiplex_enabled_) {
    ...
    // enabled if both side support multiplex for AWDL
    ...
    auto* wlan_socket = down_cast<AwdlSocket*>(virtual_socket);
    if (wlan_socket == nullptr) {
      NEARBY_LOGS(INFO) << "Failed to cast to AwdlSocket for " << service_id
                        << " with ip_address: "
                        << WifiUtils::GetHumanReadableIpAddress(ip_address);
      return ExceptionOr<AwdlSocket>(Exception::kFailed);
    }
