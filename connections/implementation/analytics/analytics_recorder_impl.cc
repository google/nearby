...
using ProtoSafeDisconnectionResult = ::location::nearby::analytics::proto::
...
ProtoSafeDisconnectionResult ConvertToProtoSafeDisconnectionResult(
    SafeDisconnectionResult result) {
  switch (result) {
    case SafeDisconnectionResult::kSafeDisconnection:
      return ConnectionsLog::EstablishedConnection::SAFE_DISCONNECTION;
    case SafeDisconnectionResult::kUnsafeDisconnection:
      return ConnectionsLog::EstablishedConnection::UNSAFE_DISCONNECTION;
    default:
      return ConnectionsLog::EstablishedConnection::
          UNKNOWN_SAFE_DISCONNECTION_RESULT;
  }
}

ConnectionsLog::OperationResultWithMedium
ConvertToProtoOperationResultWithMedium(
    const nearby::analytics::OperationResultWithMedium& cpp_result) {
  ConnectionsLog::OperationResultWithMedium proto_result;
  proto_result.set_medium(cpp_result.medium);
  if (cpp_result.update_index.has_value()) {
    proto_result.set_update_index(cpp_result.update_index.value());
  }
  proto_result.set_result_category(cpp_result.result_category);
  proto_result.set_result_code(cpp_result.result_code);
  if (cpp_result.connection_mode.has_value()) {
    proto_result.set_connection_mode(cpp_result.connection_mode.value());
  }
  return proto_result;
}
...
AnalyticsRecorderImpl::AnalyticsRecorderImpl(EventLogger* event_logger)
    : event_logger_(event_logger) {
  VLOG(1) << "Start AnalyticsRecorderImpl ctor event_logger_=" << event_logger_;
  LogStartSession();
}

AnalyticsRecorderImpl::~AnalyticsRecorderImpl() = default;

bool AnalyticsRecorderImpl::IsSessionLogged() {
  MutexLock lock(&mutex_);
  return session_was_logged_;
}

int AnalyticsRecorderImpl::GetLatestUpdateIndexLocked(
...
void AnalyticsRecorderImpl::OnStartAdvertising(
  ...
  if (!strategy.IsValid()) {
    LOG(INFO) << "AnalyticsRecorderImpl OnStartAdvertising with unknown "
                 "strategy, bail out.";
    return;
  }
  ...
  if (!advertising_metadata_params->operation_result_with_mediums.empty()) {
    for (const auto& cpp_result :
         advertising_metadata_params->operation_result_with_mediums) {
      *current_advertising_phase_->add_adv_dis_result() =
          ConvertToProtoOperationResultWithMedium(cpp_result);
    }
  }
...
int AnalyticsRecorderImpl::GetNextAdvertisingUpdateIndex() {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnStartDiscovery(
    connections::Strategy strategy, const std::vector<Medium>& mediums,
  ...
  if (!strategy.IsValid()) {
    LOG(INFO) << "AnalyticsRecorderImpl OnStartDiscovery unknown "
  ...
  if (!discovery_metadata_params->operation_result_with_mediums.empty()) {
    for (const auto& cpp_result :
      ...
      *current_discovery_phase_->add_adv_dis_result() =
          ConvertToProtoOperationResultWithMedium(cpp_result);
    }
  }
...
void AnalyticsRecorderImpl::OnStopDiscovery() {
  MutexLock lock(&mutex_);
...
int AnalyticsRecorderImpl::GetNextDiscoveryUpdateIndex() {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnStartedIncomingConnectionListening(
...
void AnalyticsRecorderImpl::OnStoppedIncomingConnectionListening() {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnEndpointFound(Medium medium) {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnRequestConnection(
...
void AnalyticsRecorderImpl::OnConnectionRequestReceived(
...
void AnalyticsRecorderImpl::OnConnectionRequestSent(
    const std::string& remote_endpoint_id) {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnRemoteEndpointAccepted(
    const std::string& remote_endpoint_id) {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnLocalEndpointAccepted(
    const std::string& remote_endpoint_id) {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnRemoteEndpointRejected(
    const std::string& remote_endpoint_id) {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnLocalEndpointRejected(
    const std::string& remote_endpoint_id) {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnIncomingConnectionAttempt(
...
void AnalyticsRecorderImpl::OnIncomingConnectionAttemptLocked(
...
void AnalyticsRecorderImpl::OnOutgoingConnectionAttempt(
...
void AnalyticsRecorderImpl::OnOutgoingConnectionAttemptLocked(
    const std::string& remote_endpoint_id, ConnectionAttemptType type,
    Medium medium, ConnectionAttemptResult result, absl::Duration duration,
    const std::string& connection_token,
    ConnectionAttemptMetadataParams* connection_attempt_metadata_params) {
...
void AnalyticsRecorderImpl::OnConnectionEstablished(
...
void AnalyticsRecorderImpl::OnConnectionClosed(const std::string& endpoint_id,
                                               Medium medium,
  ...
  if (current_strategy_session_ == nullptr) {
    VLOG(1) << "AnalyticsRecorderImpl CanRecordAnalytics Unexpected call "
...
void AnalyticsRecorderImpl::OnIncomingPayloadStarted(
...
void AnalyticsRecorderImpl::OnPayloadChunkReceived(
    const std::string& endpoint_id, std::int64_t payload_id,
...
void AnalyticsRecorderImpl::OnIncomingPayloadDone(
    const std::string& endpoint_id, std::int64_t payload_id,
...
void AnalyticsRecorderImpl::OnOutgoingPayloadStarted(
...
void AnalyticsRecorderImpl::OnPayloadChunkSent(const std::string& endpoint_id,
...
void AnalyticsRecorderImpl::OnOutgoingPayloadDone(
    const std::string& endpoint_id, std::int64_t payload_id,
    PayloadStatus status, OperationResultCode operation_result_code) {
  MutexLock lock(&mutex_);
...
void AnalyticsRecorderImpl::OnBandwidthUpgradeStarted(
...
void AnalyticsRecorderImpl::UpdateBwUpgradeNetworkInfo(
...
void AnalyticsRecorderImpl::OnBandwidthUpgradeError(
...
void AnalyticsRecorderImpl::OnBandwidthUpgradeSuccess(
...
void AnalyticsRecorderImpl::OnErrorCode(const ErrorCodeParams& params) {
  MutexLock lock(&mutex_);
  ...
  VLOG(1) << "AnalyticsRecorderImpl LogErrorCode connections_log="
          << connections_log.DebugString();  // NOLINT

  event_logger_->Log(connections_log);
}

void AnalyticsRecorderImpl::LogStartSession() {
  MutexLock lock(&mutex_);
  if (start_client_session_was_logged_) {
    LOG(WARNING) << "AnalyticsRecorderImpl CanRecordAnalytics Unexpected call "
...
void AnalyticsRecorderImpl::LogSession() {
  MutexLock lock(&mutex_);
...
bool AnalyticsRecorderImpl::CanRecordAnalyticsLocked(
    absl::string_view method_name) {
  VLOG(1) << "AnalyticsRecorderImpl LogEvent " << method_name << " is calling.";
...
void AnalyticsRecorderImpl::LogEvent(EventType event_type) {
  ConnectionsLog connections_log;
  connections_log.set_event_type(event_type);
  connections_log.set_version(kVersion);

  VLOG(1) << "AnalyticsRecorderImpl LogEvent connections_log="
          << connections_log.DebugString();  // NOLINT

  event_logger_->Log(connections_log);
}

void AnalyticsRecorderImpl::UpdateStrategySessionLocked(
...
void AnalyticsRecorderImpl::RecordAdvertisingPhaseDurationAndReasonLocked(
    bool on_stop) const {
  if (current_advertising_phase_ == nullptr) {
...
void AnalyticsRecorderImpl::FinishAdvertisingPhaseLocked() {
  if (current_advertising_phase_ != nullptr) {
...
void AnalyticsRecorderImpl::RecordDiscoveryPhaseDurationAndReasonLocked(
    bool on_stop) const {
  if (current_discovery_phase_ == nullptr) {
...
void AnalyticsRecorderImpl::FinishDiscoveryPhaseLocked() {
  if (current_discovery_phase_ != nullptr) {
...
bool AnalyticsRecorderImpl::UpdateAdvertiserConnectionRequestLocked(
...
bool AnalyticsRecorderImpl::UpdateDiscovererConnectionRequestLocked(
    ConnectionsLog::ConnectionRequest* request) {
  if (current_discovery_phase_ == nullptr) {
...
bool AnalyticsRecorderImpl::BothEndpointsRespondedLocked(
    ConnectionsLog::ConnectionRequest* request) {
  return request->has_local_response() && request->has_remote_response();
}

void AnalyticsRecorderImpl::LocalEndpointRespondedLocked(
...
void AnalyticsRecorderImpl::RemoteEndpointRespondedLocked(
    const std::string& remote_endpoint_id, ConnectionRequestResponse response) {
  auto out = outgoing_connection_requests_.find(remote_endpoint_id);
  if (out != outgoing_connection_requests_.end()) {
    ConnectionsLog::ConnectionRequest* connection_request = out->second.get();
...
void AnalyticsRecorderImpl::MarkConnectionRequestIgnoredLocked(
    ConnectionsLog::ConnectionRequest* request) {
...
bool AnalyticsRecorderImpl::ConnectionAttemptResultCodeExistedLocked(
...
bool AnalyticsRecorderImpl::EraseIfBandwidthUpgradeRecordExistedLocked(
    const std::string& endpoint_id, BandwidthUpgradeResult result,
    BandwidthUpgradeErrorStage error_stage,
    OperationResultCode operation_result_code) {
  if (current_strategy_session_ == nullptr) {
...
void AnalyticsRecorderImpl::FinishUpgradeAttemptLocked(
    const std::string& endpoint_id, BandwidthUpgradeResult result,
    BandwidthUpgradeErrorStage error_stage,
...
void AnalyticsRecorderImpl::FinishStrategySessionLocked() {
  if (current_strategy_session_ != nullptr) {
    FinishAdvertisingPhaseLocked();
    FinishDiscoveryPhaseLocked();
...
ConnectionsStrategy AnalyticsRecorderImpl::StrategyToConnectionStrategy(
    connections::Strategy strategy) {
...
void AnalyticsRecorderImpl::PendingPayload::AddChunk(
    std::int64_t chunk_size_bytes) {
  num_bytes_transferred_ += chunk_size_bytes;
  num_chunks_++;
}

ConnectionsLog::Payload AnalyticsRecorderImpl::PendingPayload::GetProtoPayload(
...
void AnalyticsRecorderImpl::LogicalConnection::PhysicalConnectionEstablished(
  ...
  if (current_medium_ != UNKNOWN_MEDIUM) {
    LOG(WARNING) << "Unexpected call to PhysicalConnectionEstablished while "
                    "AnalyticsRecorderImpl still has an active current medium.";
  }
...
void AnalyticsRecorderImpl::LogicalConnection::PhysicalConnectionClosed(
  ...
  if (current_medium_ == UNKNOWN_MEDIUM) {
    LOG(WARNING) << "Unexpected call to PhysicalConnectionClosed() for medium  "
                 << Medium_Name(medium)
                 << " while AnalyticsRecorderImpl has no active current medium";
  } else if (current_medium_ != medium) {
    LOG(WARNING) << "Unexpected call to PhysicalConnectionClosed() for medium "
                 << Medium_Name(medium)
                 << "while AnalyticsRecorderImpl has active medium "
...
void AnalyticsRecorderImpl::LogicalConnection::CloseAllPhysicalConnections() {
...
AnalyticsRecorderImpl::LogicalConnection::GetEstablisedConnections() {
  std::vector<ConnectionsLog::EstablishedConnection> established_connections;
  if (current_medium_ != UNKNOWN_MEDIUM) {
    LOG(WARNING)
        << "AnalyticsRecorderImpl expected no more active physical connections "
...
void AnalyticsRecorderImpl::LogicalConnection::IncomingPayloadStarted(
...
void AnalyticsRecorderImpl::LogicalConnection::ChunkReceived(
...
void AnalyticsRecorderImpl::LogicalConnection::IncomingPayloadDone(
    std::int64_t payload_id, PayloadStatus status,
    OperationResultCode operation_result_code) {
  if (current_medium_ == UNKNOWN_MEDIUM) {
    LOG(WARNING) << "Unexpected call to incomingPayloadDone() while "
                    "AnalyticsRecorderImpl has no active current medium.";
    return;
  }
...
void AnalyticsRecorderImpl::LogicalConnection::OutgoingPayloadStarted(
    std::int64_t payload_id, PayloadType type, std::int64_t total_size_bytes) {
  outgoing_payloads_.insert(
      {payload_id, std::make_unique<PendingPayload>(type, total_size_bytes)});
}

void AnalyticsRecorderImpl::LogicalConnection::ChunkSent(
    std::int64_t payload_id, std::int64_t size_bytes) {
...
void AnalyticsRecorderImpl::LogicalConnection::OutgoingPayloadDone(
    std::int64_t payload_id, PayloadStatus status,
    OperationResultCode operation_result_code) {
  if (current_medium_ == UNKNOWN_MEDIUM) {
...
void AnalyticsRecorderImpl::LogicalConnection::FinishPhysicalConnection(
    ConnectionsLog::EstablishedConnection* established_connection,
    DisconnectionReason reason, SafeDisconnectionResult result) {
  established_connection->set_disconnection_reason(reason);
  established_connection->set_safe_disconnection_result(
...
AnalyticsRecorderImpl::LogicalConnection::ResolvePendingPayloads(
...
OperationResultCode
AnalyticsRecorderImpl::LogicalConnection::GetPendingPayloadResultCodeFromReason(
    DisconnectionReason reason) {
  switch (reason) {
    case UPGRADED:
...
OperationResultCategory AnalyticsRecorderImpl::GetOperationResultCategory(
    location::nearby::proto::connections::OperationResultCode result_code) {
  return ConvertToOperationResultCategory(result_code);
}

void AnalyticsRecorderImpl::Sync() { MutexLock lock(&mutex_); }
