...
#include "connections/implementation/wifi_aware_bwu_handler.h"
...
#include "connections/implementation/wifi_aware_endpoint_channel.h"
...
WifiAwareBwuHandler::CreateUpgradedEndpointChannel(
  ...
  // We need to construct a NsdServiceInfo from the credentials.
  // WifiAwareMedium::Connect takes NsdServiceInfo.
...
std::string WifiAwareBwuHandler::HandleInitializeUpgradedMediumForEndpoint(
  ...
  if (wifi_aware_medium_.IsAcceptingConnections(upgrade_service_id)) {
    LOG(INFO)
        << "WifiAwareBwuHandler stopping existing accepting connections for "
        << upgrade_service_id;
    wifi_aware_medium_.StopAcceptingConnections(upgrade_service_id);
  }
  if (!wifi_aware_medium_.StartAcceptingConnections(
          upgrade_service_id,
          absl::bind_front(&WifiAwareBwuHandler::OnIncomingWifiAwareConnection,
                           this, client))) {
