...
struct ConnectionListener {
  ...
  // Called when either side has accepted or rejected the connection.
  // If the {@link ConnectionResolution}'s status is {@link
  ...
  absl::AnyInvocable<void(const NearbyDevice& remote_device,
      ...
      result_cb = [](const NearbyDevice&, ConnectionResolution) {};
  ...
  absl::AnyInvocable<void(const NearbyDevice& remote_device, Medium medium)>
      bandwidth_changed_cb = [](const NearbyDevice&, Medium) {};
};
...
struct DiscoveryListener {
  ...
  absl::AnyInvocable<void(const NearbyDevice& endpoint_id,
