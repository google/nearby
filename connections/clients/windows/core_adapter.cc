...
namespace {
void ConvertStrategy(StrategyW &strategy_w, connections::Strategy &strategy) {
  if (strategy_w == StrategyW::kNone) strategy = connections::Strategy::kNone;
  if (strategy_w == StrategyW::kP2pCluster)
    strategy = connections::Strategy::kP2pCluster;
  if (strategy_w == StrategyW::kP2pPointToPoint)
    strategy = connections::Strategy::kP2pPointToPoint;
  if (strategy_w == StrategyW::kP2pStar)
    strategy = connections::Strategy::kP2pStar;
}
}  // namespace
...
void StartAdvertising(Core *pCore, const char *service_id,
  ...
  ConvertStrategy(advertising_options_w.strategy, advertising_options.strategy);
...
void StartDiscovery(connections::Core *pCore, const char *service_id,
  ...
  ConvertStrategy(discovery_options_w.strategy, discovery_options.strategy);
...
void RequestConnection(connections::Core *pCore, const char *endpoint_id,
  ...
  ConvertStrategy(connection_options_w.strategy, connection_options.strategy);
  ...
  connection_options.enable_bluetooth_listening = false;
  connection_options.enable_webrtc_listening = false;
  ...
  connection_options.allowed.wifi_hotspot =
      connection_options_w.allowed.wifi_hotspot;
