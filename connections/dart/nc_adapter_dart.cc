...
void SendPayloadDart(NC_INSTANCE instance, int endpoint_id,
  ...
  switch (payload_dart.type) {
    ...
    case PAYLOAD_TYPE_STREAM: {
      ...
      PostResult(result_cb, NC_STATUS_PAYLOADUNKNOWN);
#else
      // NEARBY_LOGS(INFO) << "Goven222, Stream type";
      // absl::MutexLock lock(&g_sharedBuffersMutex);  // Acquire the mutex
      ...
      NEARBY_LOGS(INFO) << "Goven222, Stream type, total stream size: "
...
absl::flat_hash_map<int, std::unique_ptr<SharedBufferMapEntry>> g_sharedBuffers;
// ABSL_CONST_INIT absl::Mutex g_sharedBuffersMutex(absl::kConstInit);
...
int InitSharedBufferForPayloadDart(int64_t payload_id,
  ...
  absl::MutexLock lock(&sharedBuffer->mutex);
  ...
  sharedBuffer->payload_listener_port = payload_listener_port;
...
void StopWritingToSharedBufferDart(int64_t payload_id) {
  LOG(INFO) << "Goven222 nc_adapter_dart, StopWritingToSharedBufferDart";
  ...
  LOG(INFO) << "StopWritingToSharedBufferDart sharedBuffer found";
  ...
  LOG(INFO) << "StopWritingToSharedBufferDart sharedBuffer stopRequested";
}

bool WriteDataToSharedBufferDart(int64_t payload_id, const uint8_t *data,
                                 size_t size) {
  LOG(INFO)
      << "Goven222 nc_adapter_dart, WriteDataToSharedBufferDart, payload_id: "
      << payload_id << ", size: " << size;
  auto it = g_sharedBuffers.find(payload_id);
  if (it == g_sharedBuffers.end()) {
    LOG(INFO) << "Shared buffer not found for payload_id: " << payload_id;
    return false;
  ...
  absl::MutexLock lockSharedBuffer(
      &sharedBuffer->mutex);  // Acquire the mutex of the shared buffer
  ...
  if (!sharedBuffer->isForeRunner) {
    while (sharedBuffer->dataAvailable && !sharedBuffer->stopRequested &&
           !sharedBuffer->stopWriting) {
      LOG(INFO) << "Goven222 nc_adapter_dart, Write Wait...Wait...Wait....";
      sharedBuffer->condition.Wait(&sharedBuffer->mutex);
    }
  }

  LOG(INFO) << "Goven222 nc_adapter_dart, Leave Write Wait...Wait...Wait....";
...
int GetSharedBufferSizeDart() {
  LOG(INFO) << "GetSharedBufferSizeDart. " << g_shared_buffer_size;

  return g_shared_buffer_size;
}
#endif
