...
class NearbyShareCertificateManagerImplTest
 ...
 protected:
  ...
  void InvokeCertUploadPublishDevice(bool expected_success) {
    size_t initial_num_upload_calls =
        local_device_data_manager_->publish_device_calls().size();
    local_device_data_manager_->SetPublishDeviceResult(expected_success);
    size_t initial_num_handled_results =
        upload_scheduler_->handled_results().size();
    ...
    VerifyCertificatesUpload(/*expected_force_update_contacts=*/true);
    EXPECT_EQ(upload_scheduler_->handled_results().size(),
              initial_num_handled_results + 1);
    EXPECT_EQ(upload_scheduler_->handled_results().back(), expected_success);
  }
...
TEST_F(NearbyShareCertificateManagerImplTest,
       ForceUploadPrivateCertificates_PublishDevice_ForceUploadSuccess) {
  Initialize(/*use_identity_rpc=*/true);
  ...
  InvokeCertUploadPublishDevice(/*expected_success=*/true);
}
