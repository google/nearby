...
void OutgoingShareSession::Disconnect(const TransferMetadata& metadata) {
  ...
  disconnect_delay_timer_ = std::make_unique<ThreadTimer>(
      service_thread(), "disconnect_delay_timer", kOutgoingDisconnectionDelay,
      [connections_manager = connections_manager(),
       endpoint_id = endpoint_id()]() {
        connections_manager->Disconnect(endpoint_id);
      });
}
