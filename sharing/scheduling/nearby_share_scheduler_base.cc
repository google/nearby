  ...
  if (require_connectivity_) {
    connectivity_manager_->RegisterConnectionListener(
        ...
        [this](nearby::ConnectivityManager::ConnectionType connection_type,
          ...
          OnConnectionChanged(is_internet_connected);
        });
  }
}
...
void NearbyShareSchedulerBase::Reschedule() {
  ...
  if (!delay.has_value()) {
    ...
    LOG(INFO) << "Task \"" << pref_name_ << "\"" << " scheduled for " << *delay;
...
void NearbyShareSchedulerBase::OnConnectionChanged(bool is_internet_connected) {
