...
bool AdvertisementFilter::MatchesScanFilter(const Advertisement& adv) {
  ...
  if (!requested_identity_types.empty() &&
      !(std::find(requested_identity_types.begin(),
                  requested_identity_types.end(),
                  adv.identity_type) != requested_identity_types.end())) {
    NEARBY_LOGS(INFO)
        << "Skipping advertisement with identity type: " << adv.identity_type
  ...
  for (const auto& filter : scan_request_.scan_filters) {
    if (absl::holds_alternative<PresenceScanFilter>(filter)) {  // NOLINT
      if (MatchesScanFilter(adv.data_elements,
