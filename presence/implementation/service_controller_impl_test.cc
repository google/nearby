...
absl::string_view kAccountName = "Test account";
...
TEST_P(ServiceControllerImplTest, StartBroadcastPublicIdentity) {
  ...
  // Advertising must stop when `session` runs out of scope.
...
TEST_P(ServiceControllerImplTest, StartAndStopBroadcast) {
  ...
  EXPECT_EQ(session->StopBroadcast(), Status{Status::Value::kSuccess});
