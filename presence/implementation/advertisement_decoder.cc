...
void DecodeBaseAction(Advertisement& decoded_adv,
  ...
  ActionFactory::DecodeAction(action, decoded_adv.data_elements);
}
  ...
  for (const auto& credential : credentials) {
    ...
    if (encryptor.ok()) {
      ...
      if (result.ok() && result->size() > kBaseMetadataSize) {
        decoded_adv.public_credential = credential;
        decoded_adv.metadata_key = result->substr(0, kBaseMetadataSize);
...
absl::Status DecryptDataElements(
  ...
  decoded_adv.data_elements.emplace_back(DataElement::kSaltFieldType, salt);
  absl::string_view encrypted = elem.GetValue().substr(kSaltSize);
  absl::StatusOr<std::string> decrypted =
      DecryptLdt(decoded_adv, credentials, salt, encrypted);
  if (!decrypted.ok()) {
  ...
  while (index < decrypted->size()) {
    ...
    if (internal_elem->GetType() == DataElement::kActionFieldType) {
      DecodeBaseAction(decoded_adv, internal_elem->GetValue());
    } else {
      decoded_adv.data_elements.push_back(*std::move(internal_elem));
    }
  }
...
absl::StatusOr<Advertisement> AdvertisementDecoder::DecodeAdvertisement(
  ...
  Advertisement decoded_adv = Advertisement{};
  ...
  while (index < advertisement.size()) {
    ...
    if (IsIdentity(elem->GetType())) {
      decoded_adv.identity_type = GetIdentityType(elem->GetType());
    }
    if (IsEncryptedIdentity(elem->GetType())) {
      ...
      auto identity_type_specific_creds =
          (*credentials_)[decoded_adv.identity_type];
      absl::Status status =
          DecryptDataElements(decoded_adv, identity_type_specific_creds, *elem);
      if (!status.ok()) {
        return status;
      }
    } else {
      if (elem->GetType() == DataElement::kActionFieldType) {
        DecodeBaseAction(decoded_adv, elem->GetValue());
      } else {
        decoded_adv.data_elements.push_back(*std::move(elem));
      }
    }
  }

  return std::move(decoded_adv);
}
