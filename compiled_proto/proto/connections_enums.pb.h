...
constexpr WifiDirectAuthType WifiDirectAuthType_MAX = WIFI_DIRECT_WITH_PIN;
...
constexpr SupportedService SupportedService_MIN = SUPPORTED_SERVICE_UNSPECIFIED;
constexpr SupportedService SupportedService_MAX = SUPPORTED_SERVICE_DATA_TRANSFER;
constexpr int SupportedService_ARRAYSIZE = SupportedService_MAX + 1;

const std::string& SupportedService_Name(SupportedService value);
template<typename T>
inline const std::string& SupportedService_Name(T enum_t_value) {
  static_assert(::std::is_same<T, SupportedService>::value ||
    ::std::is_integral<T>::value,
    "Incorrect type passed to function SupportedService_Name.");
  return SupportedService_Name(static_cast<SupportedService>(enum_t_value));
}
...
constexpr DctPayloadType DctPayloadType_MIN = DCT_PAYLOAD_TYPE_UNSPECIFIED;
constexpr DctPayloadType DctPayloadType_MAX = DCT_PAYLOAD_TYPE_ONE_WAY_MESSAGE;
constexpr int DctPayloadType_ARRAYSIZE = DctPayloadType_MAX + 1;

const std::string& DctPayloadType_Name(DctPayloadType value);
template<typename T>
inline const std::string& DctPayloadType_Name(T enum_t_value) {
  static_assert(::std::is_same<T, DctPayloadType>::value ||
    ::std::is_integral<T>::value,
    "Incorrect type passed to function DctPayloadType_Name.");
  return DctPayloadType_Name(static_cast<DctPayloadType>(enum_t_value));
}
...
template <> struct is_proto_enum< ::location::nearby::proto::connections::SupportedService> : ::std::true_type {};
template <> struct is_proto_enum< ::location::nearby::proto::connections::DctPayloadType> : ::std::true_type {};
