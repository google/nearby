...
#include "location/nearby/WindowsAPI/WindowsAPITest/test_utils.h"
...
class InputFileTests : public testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    hFile_ = CreateFileA(
        test_utils::GetPayloadPathA(payloadId).c_str(),  // name of the write
    ...
    if (hFile_ == INVALID_HANDLE_VALUE) {
      NEARBY_LOG(
          ...
          test_utils::GetPayloadPathA(payloadId).c_str(), GetLastError());
    }
  ...
  void TearDown() override {
    location::nearby::PayloadId payloadId(TEST_PAYLOAD_ID);
    if (FileExists(test_utils::GetPayloadPathA(payloadId).c_str())) {
      DeleteFileA(test_utils::GetPayloadPathA(payloadId).c_str());
    }
  }
