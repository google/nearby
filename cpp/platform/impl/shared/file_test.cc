...
TEST_F(FileTest, IOFile_GetFilePath) {
  std::unique_ptr<IOFile> io_file =
      shared::IOFile::CreateInputFile(path_, GetSize());
...
TEST_F(FileTest, IOFile_NonExistentPathOutput) {
  std::unique_ptr<IOFile> io_file =
      shared::IOFile::CreateOutputFile("/not/a/valid/path.txt");
  ByteArray bytes("a", 1);
  EXPECT_TRUE(io_file->Write(bytes).Raised(Exception::kIo));
}

TEST_F(FileTest, IOFile_Write) {
  std::unique_ptr<IOFile> io_file_output =
      shared::IOFile::CreateOutputFile(path_);
  ByteArray bytes1("a");
  ByteArray bytes2("bc");
  ...
  std::unique_ptr<IOFile> io_file_input =
      shared::IOFile::CreateInputFile(path_, GetSize());
  AssertEquals(io_file_input->Read(kMaxSize), "abc");
}

TEST_F(FileTest, IOFile_CloseOutput) {
  std::unique_ptr<IOFile> io_file = shared::IOFile::CreateOutputFile(path_);
  io_file->Close();
  ByteArray bytes("a");
