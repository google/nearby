...
class FileTest : public ::testing::Test {
 protected:
  void SetUp() override {
    std::make_unique<TempPath>(TempPath::Local);
    path_ = temp_path_->path() + TEST_FILE_PARENT_FOLDER;
    full_path_ = path_ + "/" + TEST_FILE_NAME;
    file_ = std::fstream(
        full_path_, std::fstream::in | std::fstream::out | std::fstream::trunc);
  }
...
TEST_F(FileTest, InputFile_GetFilePath) {
  InputFile input_file(full_path_.c_str());
  EXPECT_EQ(input_file.GetFilePath(), full_path_);
}
...
TEST_F(FileTest, OutputFile_Write) {
  OutputFile output_file(full_path_.c_str());
  ByteArray bytes1("a");
  ByteArray bytes2("bc");
